function Test-SqlEligibility($connectionString, $sql, $fixture) {
  ...
  foreach ($case in 'no_tanks','unknown_tank','mismatched_split') {
    ...
    $drop = (Get-Body $result save_drop)[0]
    if ($drop.details.Count -ne 1 -or $drop.details[0].tank.source_id -ne $bad.site_id -or
        $drop.details[0].tank.product_source_id -ne $bad.product_id -or $drop.details[0].tank.PSObject.Properties['tank_id'] -or
        [double]$drop.details[0].quantity -ne [double]$bad.net_volume) { throw "Product fallback changed identity or measured volume: $case" }
    if ($result.requests.kind -notcontains 'status') { throw "Product fallback blocked completion: $case" }
  }
  foreach ($case in 'missing_time','blank_time','zero_net') {
    $bad = $row | Select-Object *
    switch ($case) {
  ...
  $drop = (Get-Body $result save_drop)[0]
  if ($drop.details.Count -ne 3 -or $drop.details[-1].tank.product_source_id -ne 'SECOND') { throw 'Replace-mode drop omitted product fallback beside physical allocations.' }
  $bad.drop_depart = ''
  $result = (Get-TestProjection @($row, $bad))[0]
