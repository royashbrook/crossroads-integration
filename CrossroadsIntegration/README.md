...
Delivery finishes at durable Crossroads acceptance, not downstream processing success. For update, BOL, drop, status and cancellation requests, a parsed 2xx response with status `error` or `pending` counts as `state=sent`, `status=accepted` only when its operation log contains nonblank string `_id` and `saga_id`, the expected `saga_type` for the request kind, matching `metadata.origin_order_number` and `metadata.group_id`, a nonblank `metadata.crossroads_order_id`, and exact source/destination tenant names in `log.routing`. This scoped operation evidence identifies work retained for Crossroads recovery. HTTP 200 or an error message alone is insufficient. The original response and downstream error remain in the receipt; `accepted` does not claim destination sync or completion. Creation keeps its existing synced-receipt or verified-existence gate. Transport failures, malformed responses, and responses without this evidence retain their existing classification.

Initialization applies the same evidence check to existing rejected/pending receipts, changing qualifying records to `sent/accepted` without a network request. Payloads, hashes, responses, and attempt history are preserved. The reconciliation is idempotent and releases local dependencies without replaying events already handed to Crossroads. Crossroads operators own downstream repair/retry; this module does not invoke the operator retry API. Newly changed payloads still send, and accepted receipts use the existing retention rules rather than becoming a permanent journal.
