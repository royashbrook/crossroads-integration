...
Delivery allocation slots map through `company_tankdetail.forecast_bucket` to `cmp_tank_id`; the slot number is not the tank ID. A unique, product-compatible tank assignment receives the freight's measured net quantity. Split allocations are sent only when every tank resolves and their quantities reconcile to the measured net total. Otherwise the freight uses a product-based tank key: `source_id` is the site ID, `product_source_id` is the product ID, and `tank_id` is omitted. Its quantity remains measured net volume. This delegates tank resolution to Crossroads rather than inventing an assignment. A site's replace-mode drop is omitted if any positive freight line lacks positive measured net volume or actual delivery timing. Completion payloads are generated only when required BOL and drop data are eligible.
