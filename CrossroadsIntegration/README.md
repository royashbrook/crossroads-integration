...
Dependent requests wait for an explicit `synced` create receipt for the same base URL, tenant pair, and order. A rejected create produces one result; its dependent calls are not attempted. Later runs show one `waiting_for_create` row until confirmation exists. A duplicate response proves a Crossroads record exists, not that destination creation succeeded. An empty create acknowledgment remains pending rather than confirming creation.
